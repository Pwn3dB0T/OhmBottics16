...
public class SensorBoardDiag {
	...
	public static void main(String args[]) {
		...
		while(Button.ESCAPE.isUp()) {
			Delay.msDelay(250);
			int ergebnis [] = sensor.getRed();
			LCD.clear();
			for(int i=0; i<5; i++) {
				LCD.drawInt(ergebnis[i], 0, i);
				for(int j=0; j<ergebnis[i]/1024; j++) {
					LCD.setPixel(i*8+40, 64-j, 1);
				}
			}
		}
		...
		*/
		...
		//Sensorboard.Colour c[] = sensor.getAllSensorColours();
